...
<body>
    ...
    <?php else: ?>
        <?php foreach ($students as $s): ?>
            <div class="student-bar" style="margin-bottom:18px;padding:12px;border:1px solid #ddd;">
                ...
                <button class="clear-flag-btn" onclick="clearFlag(<?= $s['id'] ?>, this)">Remove Flag</button>
            </div>
        <?php endforeach; ?>
    <?php endif; ?>
    ...
    <a href="student_space.php"><button type="button">Back to Student Space</button></a>
    ...
    function clearFlag(studentId, button) 
    {
        fetch('', {
            ...
            body: 'action=clear_flag&student_id=' + studentId
        })
        ...
        .then(data => {
            ...
            const row = button.closest('.student-bar');
            if (row) 
            {
                row.remove();
            }
            if (!document.querySelector('.student-bar')) 
            {
                ...
                msg.textContent = 'No flagged students at the moment.';
                document.body.insertBefore(msg, document.querySelector('a'));
            }
        });
    }
    </script>
